...
def color_gradient(start_hex, end_hex):
    ...
    for i in range(0, length + 1):
        interp_rgb = (int(start_rgb[0]*(length-i)/length + end_rgb[0]*i/length), int(start_rgb[1]*(length-i)/length + end_rgb[1]*i/length), int(start_rgb[2]*(length-i)/length + end_rgb[2]*i/length))
        color_dict[incr*i] = rgb_to_hex(interp_rgb)
    return color_dict
