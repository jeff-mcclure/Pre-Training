...
class RangeViewer(tk.Frame):
    ...
    def __init__(self, parent, theme):
        ...
        self.configure(background=theme.bgcolor)
        self.hand_text = font.Font(self, family='Consolas', size = 8, weight = 'bold')
        self.weight_text = font.Font(self, family='Consolas', size = 6, weight = 'bold')
        self.colors = color_gradient(theme.bgcolor, '#fcf803')
        ...
        self.range_canvas = tk.Canvas(self, width=540, height=511, bg=theme.bgcolor, borderwidth=0, highlightbackground="black", highlightcolor="black", highlightthickness=0, bd=0)
     
        self.range_canvas.bind('<B1-Motion>', lambda e: self.colorset(theme, e))
        self.range_canvas.bind('<Button-1>', lambda e: self.colorsetclick(theme, e))
        ...
        self.perclabel = tk.Label(self, bg=theme.bgcolor, textvariable=self.percent, borderwidth=0, highlightthickness=0, highlightbackground='black', font=self.hand_text, relief="solid", fg=theme.fcolor)

        self.range_canvas.grid(row=0, column=0, sticky='NSEW', padx=0, pady=0)
        self.perclabel.grid(row=1, column=0, sticky='W')

        self.rowconfigure(0, weight=10, uniform='x')
    ...
    def resize(self, event):
        ...
        if w > 0:
            self.hand_text['size'] = min(11, int(0.028*min(w,h)))
            self.weight_text['size'] = min(11, int(0.028*min(w,h)))
            for j in range(0, 13):
                for i in range(0, 13):
        ...
        if not self.lock:
            ...
            if idx_x in range(0,13) and idx_y in range(0,13):
                if not self.erasemode:
                    ...
                    self.range_canvas.itemconfig(self.rectgrid[idx_x][idx_y], fill=theme.bgcolor)
                    self.weightStrs[idx_x][idx_y] = self.weightStrs[idx_x][idx_y].split('\n')[0] + '\n0'
                    self.range_canvas.itemconfig(self.handgrid[idx_x][idx_y], text=self.weightStrs[idx_x][idx_y].split('\n')[1])
        self.percent.set(f'{RangeViewer.get_percentage(self):.1f}' + '%')

    def colorsetclick(self, theme, event):
        if not self.lock:
            idx_x = 13*(self.winfo_pointerx() - self.winfo_rootx()) // self.range_canvas.winfo_width()
            idx_y = 13*(self.winfo_pointery() - self.winfo_rooty()) // self.range_canvas.winfo_height()
            if idx_x in range(0,13) and idx_y in range(0,13):
...
class CallViewer(RangeViewer):
    def __init__(self, parent, theme):
        ...
        self.colors = color_gradient(theme.bgcolor, '#008080')

class RaiseViewer(RangeViewer):
    def __init__(self, parent, theme):
        super().__init__(parent, theme)
        self.colors = color_gradient(theme.bgcolor, '#FF0000')
